...
<<>>=
model5 <- mhurdle(durable~0|age+quant|age+quant, tobin, dist="n", method="bfgs")
@ 
...
To remove the \code{age} variable from the selection equation and to
add the square of the \code{age} to the regression equation, one can
update the model the following way:

<<>>=
model8ibus <- update(model8i, . ~ . - age | . + I(age^2)| .)
@ 
...
<<>>=
vuongtest(model6i, model8d) 
@ 

In the previous example, the Vuong test is unable to discriminate
