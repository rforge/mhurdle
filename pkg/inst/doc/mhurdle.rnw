...
% of a PhD dissertation carried out by Stephane Hoareau
% \cite{Hoareau/09} at the University of La Reunion under the
...
\Abstract{ \pkg{mhurdle} is a package for \proglang{R} enabling the
  estimation of a wide set of models for which the response is
  left censored at zero. This kind of models are called limited dependent or
...
a large proportion of zeros due to many households recording, for one
reason or another. no expenditure for some items.
...
observations. More specifically, three main decision mechanisms have been put 
forward, in applied microeconometrics, to explain the appearence of such zero
observations.

\begin{description}
\item[A good selection mechanism] . According to this mechanism, the
consumer first decides which goods to include in its choice set and,
as a consequence, he can discard some marketed goods because he dislikes them
(like meet for vegeterians or wine for non drinker people) or he considers 
them harmful (like alcohol, cigarettes, inorganic food, holidays in
dangerous countries), among others.

\item[A desired consumption level mechanism] . According to this mechanism,
once a good has been selected, the consumer decides at which level he
desires to consume it and, as a consequence of consumer's preferences,
resources and selected good prices, its rational decision can turn 
out to be a negative desired consumption level leading to a nil consumption.  

\item[A purchase mechanism] . According to this mechanism, once a 
consumption decision has been taken, the consumer set up the schedule
at which to buy the good and, as a consequence of its buying strategy 
zero expenditure may be observed if the survey is carried out over a 
too short period with respect to the frequency at which the good is bought.
\end{description}

The original Tobin's model takes only the second source of zeros into account. 

With \pkg{mhurdle}, up to three sources of zero may be
...
variable is left censored at zero and the observations' sample
...
explanatory variables on the latent dependent variables $y_1^*$, $y_2^*$,
...
mechanisms is not in effect. For instance, we know in advance that all
...
from this general econometric framework by enforcing censoring mechanisms 
not in effect and by selecting an appropriate functional form
...
econometric literature. In particular, models 3 and 4 are single-hurdle 
good selection models originated by \citet{CRAGG/71}. The 
double-hurdle model combining uncorrelated good selection and lack of
...
Model 12 is an original triple-hurdle model originated in
...
\ln L_i^- = \left\{
  \begin{array}{ll}
    \ln \left(1 - \Phi_B \left(\beta_1^\top x_{1i},\frac{\beta_2^\top x_{2i}}{\sigma};\rho \right)
    ...
    \ln \left(1 - \frac{\Phi_B \left(\beta_1^\top x_{1i},\frac{\beta_2^\top x_{2i}}{\sigma};\rho\right)}
      {\Phi\left(\frac{\beta_2^\top x_{2i}}{\sigma}\right)} \Phi(\beta_3^\top x_{3i}) \right) & \mbox{for truncated-normal models} \\
\end{array}
\right.
$$

where $\Phi(z)$ denotes the cumulative distribution function of a standard normal random
variable and $\Phi_B (z_1,z_2;\rho)$ the cumulative distribution function of two
dependent standard normal random variables with correlation coefficient $-1<\rho<1$.
We remind that log-normal and truncated normal models assume
that the lack of resources mechanism is not in effect, whereas it is
in effect in normal models.
...
$$
\Phi_B \left(\beta_1^\top x_{1i},\frac{\beta_2^\top x_{2i}}{\sigma};0
...
To illustrate the use of \pkg{mhurdle}, let's use the \code{tobin}
...
To illustrate the use of \pkg{mhurdle} package, we first estimate an
independent triple-hurdle model, which we call \code{model12i} :

<<>>=
model12i <- mhurdle(durable~age+quant|age+quant|age+quant, data = tobin, dist="n", method="nr")
@

In applied work, the issue may be to select the relevant hurdles.  As
an alternative to the previously estimated triple-hurdle model we can
now estimate more \textit{a priori} restricted models where only one or two
...
<<>>=
model5 <- mhurdle(durable~0|age+quant|0, data = tobin, dist="n", method="nr")
@
...
<<>>=
model3i <- mhurdle(durable~age+quant|age+quant|0, data = tobin, dist="l")
@
...
<<>>=
model6 <- mhurdle(durable~0|age+quant|age+quant, data = tobin, dist="l")
@
...
<<>>=
model8i <- mhurdle(durable~age+quant|age+quant|0, data = tobin, dist="n")
@
...
<<>>=
model8d <- update(model8i, corr = 'sel')
@
