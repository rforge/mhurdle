...
To illustrate the use of \pkg{plm}, , we use two surveys conducted by
the Bureau of Labour Statistics of the U.S. Department of Labour. The
first survey, called "Dairy Survey", collects data from 10’351
households on frequently purchased goods, especially food, on a two
weeks basis. The second survey, called "Interview Survey", collects
data from 25’813 households on all expenditure, on a quarterly
basis. The micro-data 16files are publicly available on the website of
the Bureau of Labour Statistics, and may be downloaded and used
without permission.

<<>>=
data("Intview", package = "mhurdle")
head(Intviewu, 3)
mean(Intview$vacations == 0)
max(Intview$vacations)
@
...
\begin{description}
\item[income] the net income,
\item[uc] the number of consumption units,
\item[urban]{does the household live in an urban area (\code{yes} or \code{no}),}
\item[smsa]{does the household live in a SMSA (\code{yes} or \code{no}),}
\item[region]{the region of residence of the household, one of \code{northeast}, \code{midwest}, \code{south} and \code{west},}
\item[age]{the age of the reference person of the household,}
\item[educ]{the number of year of education of the reference person of the household,}
\item[sex]{the sex of the reference person of the household (\code{male} and \code{female}),}
\item[race]{the race of the head of the household, one of \code{white}, \code{black}, \code{indian}, \code{asian}, \code{pacific} and \code{multirace},}
\item[hispanic]{is the reference person of the household is hispanic (\code{no} or \code{yes}),}
\item[size]{the number of persons in the household,}
\item[less18]{the number of persons less than 18 years old in the household,}
\item[income]{the income of the household for the 12 month before the interview,}
...
<<>>=
Intview$incu <- with(Intview, income / cu)
Intview$incum <- with(Intview, incu / mean(incu))


m010 <- mhurdle(vacations ~ 0 | log(incum) + I(log(incum)^2) +
                I(log(incum)^3) + age  + gender + educ +
                size| 0, data = Intview, dist = "n", method = 'bfgs')
@
...
m111dii <- mhurdle(comics ~ gender + educ  |  log(incum) +
                   ...
                   data = Comics, corr = TRUE, dist = "n", method = 'bhhh', iterlim = 200)
@
