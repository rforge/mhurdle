...
\Keywords{households' expenditure survey analysis, censored regression
  models, hurdle models, maximum likelihood estimation, nonlinear
  goodness of fit measures, Vuong tests for model selection, Tobit
  models, \proglang{R}}

\Plainkeywords{households' expenditure survey analysis, censored
  regression models, hurdle models, maximum likelihood estimation,
  nonlinear goodness of fit measures, Vuong tests for model selection,
  Tobit models, R}
...
\citet{SMITH:02}, table~\ref{tab:exconso} gives an overview of these
studies and clearly shows that in empirical works, most of the time,
two mechanisms are taken into account simultaneously.
...
\citep[see][]{MOFF:05}, sport activities \citep[][]{HUMP:RUSE:10},
...
\ref{sec:examples} illustrates the use of \pkg{mhurdle} with
...
\item Hurdle 2 models the household decision of consuming or not
  ...
  and a truncated normal functional form where $y^*_2$ is generated by
  the second of the set of linear relationships (\ref{eq1}) with
  $\epsilon_2$ distributed according to a normal random disturbance
  left-truncated at $\epsilon_2 = -\beta_2^\top x_2$, as suggested by
  \citet{CRAGG/71}. Nevertheless, to avoid a cumbersome analytic
  presentation of our models, in the following we only consider the
  log-normal model specification.
...
dealt with by means of the \pkg{mhurdle} package. \textcolor{red}{In
  the following, we'll name the different models using the structure
  of this figure. 1 will indicate that a specific hurdle is operative
  as a 0 means that it is not operative. For example, \code{011} is the model
  for which there is no selection process, but for which zero occurs
  because of corner solutions and infrequency of purchase.}
...
econometric literature. In particular, model \code{100} is a single-hurdle
good selection model originated by \citet{CRAGG/71}. The double-hurdle
model \code{110} combining independent good selection (hurdle 1) and desired
...
\item Models \code{111} and \code{101} are trivariate hurdle models as they involve
  ...
  respectively.  As the unit of measurement of $\epsilon_1$ and
  $\epsilon_3$ are not identified, these disturbances are normalised
  by setting their variances equal to 1.
...
\item The probability mass $P(y=0)$ for bivariate hurdle models
  ...
  bivariate hurdle models \code{001} and \code{100} can be derived
...
not a effect or a zero correlation.
...
\item The density function $f_+(y)$ for bivariate hurdle models
  \code{011} and \code{110} and univariate hurdle model 3 can be
  derived from that of trivariate model \code{111} by eliminating
...
squares using the sub-sample of uncensored observations and
...
observations using $y_i\Phi(\hat{\beta}_3^{\top}x_{3i})$ or $\ln y_i
+\ln \Phi(\hat{\beta}_3^{\top}x_{3i})$ (in the case of a log-normal
specification) as dependent variable and assuming no correlation
...
numerous set of competing models.
