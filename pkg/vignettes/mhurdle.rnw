...
H3D <- mhurdle(shows ~ educ + size | linc + linc2 | age + age2 + smsa, 
               Interview, dist = "ln2", corr = TRUE, method = "bhhh")
@ 
...
A three equation - two hurdles model is obtained by choosing a
distribution only defined for positive values of the response, 
\emph{e.g.} the \code{ln} distribution.

<<label = TwoHurdlesThreeEqs>>=
H2D <- update(H3D, dist = "ln")
@ 
